<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="bg-gradient-to-r from-[#0D47A1] to-[#90CAF9] rounded-xl p-6 shadow-lg" v-motion="{
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-4">
          <Sel-ect :size="'large'" v-model="selectedSession" :options="sessions" optionLabel="name"
            placeholder="Select Session" class="w-full md:w-[300px] shadow-md" :pt="{
    ...
    <div v-if="selectedSession && fee" class="grid grid-cols-1 md:grid-cols-3 gap-4" v-motion="{
      initial: { opacity: 0 },
      enter: {
        opacity: 1,
    ...
    <div v-if="selectedSession && fee" class="bg-white rounded-xl p-5 shadow-md" v-motion="{
      initial: { opacity: 0, y: 20 },
    ...
    <transition name="fade-zoom" mode="out-in">
      <div v-if="!selectedSession"
        class="bg-white flex items-center justify-center h-[60vh] rounded-xl p-6 overflow-hidden shadow-md" v-motion="{
          initial: {
            opacity: 0,
            y: 20,
            scale: 0.95,
        ...
        <div class="flex flex-col gap-4 items-center text-center">
          <div class="relative">
            ...
            <div class="bg-[#E9F5FF] p-6 rounded-full relative overflow-hidden">
              <svg class="w-16 h-16 text-[#0D47A1] relative z-10" xmlns="http://www.w3.org/2000/svg" fill="none"
                viewBox="0 0 24 24" stroke="currentColor" v-motion="{
              ...
              <div class="absolute inset-0 bg-[#90CAF9]/30 -z-0" v-motion="{
          ...
          <h1 class="text-2xl font-semibold text-gray-700" v-motion="{
          ...
          <p class="text-lg text-gray-500 max-w-md" v-motion="{
            initial: { opacity: 0, y: 10 },
            enter: {
              opacity: 1,
              y: 0,
          ...
          <div class="mt-6 text-[#0D47A1] flex items-center gap-2 bg-[#E9F5FF] px-4 py-2 rounded-full shadow-sm"
            v-motion="{
            ...
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 animate-bounce-vertical" fill="none"
      ...
      <div v-else class="flex flex-col gap-6">
        ...
        <div class="bg-gradient-to-r from-[#0D47A1] to-[#90CAF9] rounded-xl p-6 shadow-lg relative overflow-hidden"
          v-motion="{
            initial: { opacity: 0, y: 20 },
          ...
          <div class="flex flex-col md:flex-row justify-between items-center gap-6 relative z-10">
            ...
            <But-ton @click="downloadInvoice" label="Download Invoice" icon="pi pi-download"
              class="bg-white text-blue-700 border-none shadow-md hover:shadow-lg transition-shadow" :pt="{
        ...
        <FeeData :name="fee?.student?.name" :level="fee?.level?.title" :matric="fee?.student?.username"
          :session="selectedSession?.name" :faculty="fee?.student?.programme?.department?.faculty?.name"
          :email="fee?.student?.email" :department="fee?.student?.programme?.department?.name"
          :programme="fee?.student?.programme?.name" :date="formattedDate" :feeItems="fee?.feeItems"
          :feePayment="fee?.feePayment" :loading="loading" />
      </div>
    </transition>
  </div>
</template>
