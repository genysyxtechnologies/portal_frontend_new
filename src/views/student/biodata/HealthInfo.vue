<template>
  <div class="container mx-auto p-8 bg-white h-full rounded-xl shadow-md">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <!-- Bloog group -->
      <div>
        <label for="bloodGroup" class="block text-sm font-medium text-gray-700 mb-1 labels">Blood Group</label>
        <div class="relative">
          <Sel-ect :size="'large'" v-model="selectedBloodGroup" :options="[user?.information.bloodGroup]"
            optionLabel="name" placeholder="Select Blood Group" class="card w-full"
            :disabled="!editableFields.bloodGroup" />
          <EditToggle :is-editing="editableFields.bloodGroup" @click="toggleEdit('bloodGroup')"
      ...
      <!-- Genotype -->
      <div>
        <label for="genotype" class="block text-sm font-medium text-gray-700 mb-1 labels">Genotype</label>
        <div class="relative">
          <Sel-ect :size="'large'" v-model="selectedGenotype" :options="[user?.information.genotype]" optionLabel="name"
            placeholder="Select Genotype" class="card w-full" :disabled="!editableFields.genotype" />
          <EditToggle :is-editing="editableFields.genotype" @click="toggleEdit('genotype')"
            class="absolute right-3 top-3" />
        </div>
      </div>
